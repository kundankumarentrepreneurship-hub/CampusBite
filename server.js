...
async function init(){
  ...
  try{
    ...
    // Demo accounts.
    await client.query(`DELETE FROM customers WHERE customer_code NOT LIKE 'CB%'`);
    const customers=[
      ['CB2026001','Aarav Sharma','','Campus@123','Teacher'],
      ['CB2026002','Diya Nair','','Campus@456','Student']
    ];
    for(const [reg,name,year,pw,role] of customers){
      const hash=await bcrypt.hash(pw,10);
      await client.query(
        `INSERT INTO customers(customer_code,name,year,password_hash,role)
         VALUES($1,$2,$3,$4,$5)
         ON CONFLICT(customer_code) DO UPDATE SET name=EXCLUDED.name,year=EXCLUDED.year,password_hash=EXCLUDED.password_hash,role=EXCLUDED.role`,
        [reg,name,year,hash,role]
      );
      await client.query(`INSERT INTO wallet_transactions(customer_id,icon,title,sub,amount,type)
    ...
    for(const shop of shops){
      ...
      await client.query(`INSERT INTO staff_users(name,shop,pin_hash) VALUES($1,$2,$3) ON CONFLICT(shop) DO NOTHING`,[shop+' Staff',shop,hash]);
    }
    const menu=[
...
app.post('/api/auth/customer',async(req,res)=>{
  try{const {customerCode,password,role:loginRole}=req.body; const r=(await (await db()).query('SELECT * FROM customers WHERE customer_code=$1',[String(customerCode||'').trim()])).rows[0];
    if(!r || !(await bcrypt.compare(String(password||''),r.password_hash)) || String(loginRole||'').trim().toLowerCase() !== String(r.role||'').trim().toLowerCase()) return res.status(401).json({error:'Invalid customer ID, password, or role'});
    const token=tokenFor({role:'customer',id:r.id,customerCode:r.customer_code});
    res.json({token,user:{role:'customer',name:r.name,customerCode:r.customer_code,year:r.year,profileRole:r.role||'Student'}});
